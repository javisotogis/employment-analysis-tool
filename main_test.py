...
def main():
    ...
    for query in queries:
        for location in locations:
            job_df = get_adzuna_jobs(query, location, total_results=100)
            all_data.append(job_df)  # Append each DataFrame

    if all_data:
        combined_df = pd.concat(all_data, ignore_index=True)
        os.makedirs("tmp_outputs", exist_ok=True)
        combined_df.to_csv("tmp_outputs/adzuna_all_jobs.csv", index=False)
        print(f"✅ Exported {len(combined_df)} jobs to tmp_outputs/adzuna_all_jobs.csv")
    else:
